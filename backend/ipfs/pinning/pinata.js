...
// backend/internal/ipfs/pinning/pinata.js
// Pinata IPFS Pinning Service Client
...
class PinataClient {
  ...
  async pinFileFromFS(filePath, options = {}) {
    ...
    return this.pinFileFromStream(fileStream, path.basename(filePath), options);
  }
   ...
   */
  async unpin(hash) {
    const response = await this.client.delete(`/pinning/unpin/${hash}`);
  ...
  async updatePinMetadata(hash, metadata) {
    const response = await this.client.put(`/pinning/hashMetadata`, {
      ipfsPinHash: hash,
      name: metadata.name,
  ...
  async pinDIDDocument(didDocument, did) {
    ...
    return this.pinJSON(didDocument, options);
  }
  ...
  async pinCredential(credential, holderDID, credentialType) {
    ...
    return this.pinJSON(credential, options);
  }
  ...
  async deleteWebhook(webhookId) {
    const response = await this.client.delete(`/psa/webhooks/${webhookId}`);
